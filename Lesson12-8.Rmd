...
for(i in 1:Monte)
{
  ...
  monte.root <- sqrt(n)*(mean(monte.sample) - mau.mean)
	monte.roots <- c(monte.roots,monte.root)
}
interval <- c(sort(monte.roots)[floor(.025*Monte)],sort(monte.roots)[floor(.975*Monte)])
```

- The $95 \%$ confidence interval based on the bootstrap is [`r mau.mean - interval[2]/sqrt(n)`,`r mau.mean - interval[1]/sqrt(n)`].
